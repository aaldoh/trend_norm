...
knitr::opts_chunk$set(echo = TRUE, cache = TRUE, warning = FALSE, message = FALSE, autodep = TRUE)
...
A total of `r data_desc[[1]]` participants were recruited through a survey posted on Prolific. `r data_desc[[2]]` were excluded due to incomplete responses, and `r data_desc[[3]]` were excluded as they are vegan/vegetarian. Using a robust Mahalanobis distance based on the Minimum Covariance Determinant, `r nrow(clean) - nrow(no_out)` multivariate outliers were detected and removed. The final sample (*N* = `r data_desc[[4]]`) ranged in age `r age_desc$min_age` to `r age_desc$max_age` (*M~age~* = `r age_desc$m_age`, *SD* = `r age_desc$sd_age`). The participants were predominantly female (`r gender_freq[2]`%). The participants received £0.35 ($0.45) for successfully completing the task.
...
measure.tib %>%
  kable(caption = "Means (M), standard deviations (SD), and inter-correlations among study measures.", col.names = c("Measure", "$M$", "$SD$", 1:6), digits = 2, escape = F) %>% 
...
```{r h1plots, cache=TRUE, fig.cap = "Violin plots of outcome variables by condition"}
out_plots
```
